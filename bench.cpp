...
static const std::vector<int>& mnist_data()
{
	static std::vector<int> data{
		[]() // immediately-invoked lambda
		{
			std::vector<int> tmp;
			tmp.reserve(54000);
			get_mnist_labels(std::back_inserter(tmp));
			return tmp;
		}()
	};
	return data;
...
void string_uniform_init(std::string& str)
{
	static std::uniform_int_distribution<char> char_gen(char(32), char(126));
	static std::uniform_int_distribution<std::size_t> size_gen(5, 200);
}
